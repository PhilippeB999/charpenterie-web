...
const STORAGE_KEY = "charpenteriequest_state_v1";
...
const ACCESS_CODE = "CHARP-2026-8TJD"; // code fourni au centre de formation après achat de la licence
...
const SYNC_KEY = "charpenteriequest_sync_pending";
...
function deviceId() {
  let id = localStorage.getItem("charpenteriequest_device_id");
  if (!id) {
    id = "cq-" + (crypto.randomUUID ? crypto.randomUUID() : Date.now().toString(36) + Math.random().toString(36).slice(2));
    localStorage.setItem("charpenteriequest_device_id", id);
  }
  return id;
}
